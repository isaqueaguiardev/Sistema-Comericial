...
from config import APP_NAME, APP_VERSION, EMPRESA, CORES, DATABASE_PATH
...
if EMPRESA.get("logo"):
    st.sidebar.image(EMPRESA["logo"], use_container_width=True)
else:
    st.sidebar.markdown(f"## {EMPRESA.get('icone', '🏪')} {EMPRESA['nome']}")

st.sidebar.caption(EMPRESA.get("slogan", "Sistema de gestão"))
st.sidebar.markdown("---")
...
st.sidebar.markdown(f"**Cidade:** {EMPRESA['cidade']} - {EMPRESA['estado']}")
...
<div class="hero-title">
    {EMPRESA.get("saudacao_dashboard", f"Olá, {EMPRESA['nome']} 👋")}
</div>

<div class="hero-subtitle">
    {EMPRESA.get("mensagem_dashboard", "Resumo atualizado do seu negócio.")}
</div>
    </div>
""", unsafe_allow_html=True)
    ...
    margin-bottom:20px;
    ">
    Meta calculada automaticamente pelo sistema
    </div>
    ...
    <div>
    <b>Meta diária</b><br>
    R$ {meta['media_diaria']:.2f}
    </div>

    <div>
    <b>Dias restantes</b><br>
    {meta['dias_meta_restantes']}
    </div>
    ...
    with col1:
        card_indicador(
          "💰",
          "Faturamento",
          dinheiro(indicadores["faturamento"]),
    ...
    with col2:
        card_indicador(
          "📈",
          "Lucro bruto",
          dinheiro(indicadores["lucro"]),
          "Venda menos custo dos produtos",
        )
    ...
    with col4:
        card_indicador(
          "🎟️",
          "Ticket médio",
          dinheiro(indicadores["ticket_medio"]),
          "Média por pedido",
        )
    ...
    st.markdown(
        '<div class="footer-note">Airesbella · Dashboard executivo conectado aos módulos do sistema</div>',
        unsafe_allow_html=True,
    )
